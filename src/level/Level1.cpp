...
void Level1::update(float dt) {
    ...
    if (m_levelWon) {
        ...
        if (!m_winMusicPlayed) {
            ...
            int rewardCoins = (m_levelNumber == 1 ? 1000 : (m_levelNumber == 2 ? 1500 : 2000));
...
void Level1::drawWinScreen() {
    ...
    if (m_winTimer >= 1.0f) {
        ...
        if (m_brianneLoaded) {
            m_brianneFont.DrawTextCentered("You defeated all zombies and saved your lawn!", subRect, 0.95f, ColorAlpha(WHITE, bannerAlpha));
        } else {
            DrawText("You defeated all zombies and saved your lawn!", 225, 400, 17, ColorAlpha(WHITE, bannerAlpha));
        }

        // 3. Cash Reward Line: with BrianneTod16 in bright coin gold
        int rewardCoins = (m_levelNumber == 1 ? 1000 : (m_levelNumber == 2 ? 1500 : 2000));
        std::string rewardStr = (m_levelNumber == 1 ? "+1,000 Coins Earned!" : (m_levelNumber == 2 ? "+1,500 Coins Earned!" : "+2,000 Coins Earned!"));
