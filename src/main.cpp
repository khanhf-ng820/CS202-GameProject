...
enum class GameState {
    MainMenu,
    PlantAnimTest  // The existing plant animation visualizer
};

int main() {
    // Enable config flags for high quality drawing
    SetConfigFlags(FLAG_MSAA_4X_HINT);
    InitWindow(1280, 768, "Plants vs. Zombies - C++ & Raylib");
    SetTargetFPS(60);
    ...
    // --- Plant Animation Test State (lazy-initialized on first entry) ---
    bool plantTestInitialized = false;
    ...
    while (!WindowShouldClose() && !shouldQuit) {
        ...
        case GameState::MainMenu: {
            ...
            if (action == MenuAction::StartAdventure) {
                currentState = GameState::PlantAnimTest;
                // Initialize plant test if first time
                if (!plantTestInitialized) {
                    currentPlant = std::make_unique<FirePea>(res, 550, 420);
                    plantTestInitialized = true;
                }
            }
            else if (action == MenuAction::Quit) {
                shouldQuit = true;
            }
            // Level1/2/3, Options, Help are stubs — do nothing
            break;
        }

        // =============================================================
        // PLANT ANIMATION TEST STATE (existing code, unchanged logic)
        // =============================================================
        case GameState::PlantAnimTest: {
            // --- Update ---
            ...
            // Back to Menu button
            if (DrawButton({ 20, (float)GetScreenHeight() - 45.0f, 120, 35 }, "< MENU",
