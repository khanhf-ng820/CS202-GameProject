...
*   **Zero-Allocation Track Rendering:** In `Reanimation::Draw()` and `GetTrackBounds()`, never copy `track.keyframes[j].imageName` into a local `std::string` value. Always resolve image names using `const std::string*` pointers directly into the keyframe array to avoid per-frame heap copies.
*   **Stack Buffers & SSO for Texture Lookups:** In `Resources::GetTexture()`, never allocate dynamic `std::string` objects to strip prefixes or convert to uppercase. Use a `char[128]` stack buffer and take advantage of macOS's 22-byte Small String Optimization (SSO) so that constructing the final lookup key `std::string` avoids `malloc`.
*   **Packed Integer Font Kerning:** Never allocate `std::string` pairs (e.g., `std::string pair = {ch, nextCh}`) inside `BitmapFont::DrawText()` or `MeasureText()` for kerning lookups. Always use a packed 16-bit integer map `std::unordered_map<uint16_t, int>` where the key is `((uint16_t)(uint8_t)c1 << 8) | (uint16_t)(uint8_t)c2`.
*   **Lazy UI Asset Caching:** Never call `res.GetTexture("...path...")` repeatedly with string literals inside `draw()` loops for static backgrounds or UI elements. Lazily cache the `Texture2D` in the class header (e.g., `mutable Texture2D m_bgTex;`) to bypass dynamic string construction entirely.
...
*   **Zero-Allocation Loop Computation:** Never dynamically allocate `std::map` or `std::vector` inside hot-loop geometry computation like `GetLoopStartTime`. Use bounded static-sized stack arrays (e.g., `int frameCounts[512] = {0};`) to tally animation frame data.
